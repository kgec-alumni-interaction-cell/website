...
function Verify({ alumnis, students }: Props) {
  ...
  return (
    <Layout>
      <ul className="grid grid-cols-2 gap-0 text-base font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
        <li className="">
          <button
            onClick={() => {
                      ...
                      onClick={async () => {
                        const response = await fetch(
                          "https://kgec-alumni-backend.onrender.com/users/verify",
                          {
                            ...
                            body: JSON.stringify({
                              id: user.id,
                              token: "b863fcc7a0f0be7c2fc1b83c2bd55c16",
                            }),
                          }
                        );
